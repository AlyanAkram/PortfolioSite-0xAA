...
const LINES = [
  ...
  {
    text: "│  ALYAN AKRAM  ·  DEVELOPER PORTFOLIO                │",
    cls: "header",
  },
  {
    text: "└──────────────────────────────────────────────────────┘",
...
// Type several characters at once.
// This keeps the typewriter effect while making it much faster.
const CHARS_PER_TICK = 5;
const TYPE_INTERVAL = 12;

// Almost no pause between lines.
const LINE_PAUSE = 5;
...
export default function Terminal({ onDone }) {
  ...
  useEffect(() => {
    ...
    // Instead of waiting for every character individually,
    // we type 3 characters every 12ms.
    async function typeText(el, text) {
      for (let i = 0; i < text.length; i += CHARS_PER_TICK) {
        if (cancelled) return;

        el.textContent += text.slice(i, i + CHARS_PER_TICK);
    ...
    async function runProgress() {
      ...
      await new Promise((resolve) => {
        ...
        const interval = setInterval(() => {
          ...
          // Fill 5 blocks per tick.
          i += 5;

          bar.textContent =
            "█".repeat(Math.min(i, total)) + "░".repeat(Math.max(total - i, 0));
    ...
    async function run() {
      ...
      if (!cancelled && !doneRef.current) {
        ...
        // Tiny pause before entering the portfolio.
        await sleep(100);
